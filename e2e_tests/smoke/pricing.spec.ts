...
/**
 * smoke（真相源 · 真实 dev 后端）：/pricing（官网定价页）。
 *
 * 该路由**无需登录**——pricing 是公开内容（见 page.tsx：RSC getFullLLMModels 取公开模型目录；
 * GPU/Sandbox tab 客户端取公开定价）。实测未登录可达、不弹 /login。故**不加 token skip**，随时可跑。
 *
 * 断言策略（truth-source = 真实后端契约仍活着 + 页面结构 + 不崩）：
 ...
 *    未弹登录、不触发错误边界、官网外壳（无 console 侧边导航）。
 *  - **不**断言 GPU 卡片具体出行——真实 dev 目录数据稀疏（usableNode=false 等导致 GPU 价格表
 *    未必出卡），且具体数据值断言已在 hermetic 层（注入 prod 形状 fixture）覆盖；smoke 只保证
 *    「端点活着 + 页面结构 + 不崩」。
 */
test.describe("@smoke Pricing（真实后端）", () => {
  test("真实后端公开定价端点 2xx 且官网定价页结构渲染、不崩", async ({
    page,
  }) => {

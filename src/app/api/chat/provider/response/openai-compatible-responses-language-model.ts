...
import { convertToOpenAIResponsesInput } from "./convert-to-openai-compatible-responses-input";
import { mapOpenAIResponseFinishReason } from "./map-openai-compatible-finish-reason";
import {
...
import { prepareResponsesTools } from "./openai-compatible-prepare-tools";

export class OpenAICompatibleResponsesLanguageModel implements LanguageModelV2 {
  ...
  async doGenerate(
    ...
    const {
      ...
      successfulResponseHandler: createJsonResponseHandler(
        openaiResponsesResponseSchema as any,
      ),
    ...
    for (const part of (response as any).output) {
      switch (part.type) {
        case "reasoning": {
    ...
    const providerMetadata: SharedV2ProviderMetadata = {
      provider: { responseId: (response as any).id },
    };

    if (logprobs.length > 0) {
      providerMetadata.provider.logprobs = logprobs;
    }

    if (typeof (response as any).service_tier === "string") {
      providerMetadata.provider.serviceTier = (response as any).service_tier;
    }

    return {
      content,
      finishReason: mapOpenAIResponseFinishReason({
        finishReason: (response as any).incomplete_details?.reason,
        hasFunctionCall,
      }),
      usage: {
        ...
        reasoningTokens:
          (response as any).usage.output_tokens_details?.reasoning_tokens ??
          undefined,
        cachedInputTokens:
          (response as any).usage.input_tokens_details?.cached_tokens ??
          undefined,
      },
      request: { body },
      response: {
        id: (response as any).id,
        timestamp: new Date((response as any).created_at * 1000),
        modelId: (response as any).model,
        ...
        >({
                ...
                for (const summaryIndex of Object.keys(
                  ...
                  if (
                    ...
                    controller.enqueue({
                      ...
                      providerMetadata: { provider: { itemId: value.item_id } },
                    });
                    activeReasoningPart.summaryParts[summaryIndex] =
                      "concluded";
                  }
          ...
          flush(controller) {
            ...
            if (serviceTier !== undefined) {
              providerMetadata.provider.serviceTier = serviceTier;
            }

...
import { minimaxVideo01, getFailMsgV3, checkProgressV3 } from "@/api/api";
...
const DB_KEY_TASK_ID = "minimax_video_01_task_id";
const DB_KEY_TASK_PROGRESS = "minimax_video_01_task_progress";
const deleteTask = () => {
...
const useCases = defaultCases[FUNC_NAME.MINIMAX_VIDEO_01];
export default function MinimaxVideo01({
  apiKey,
  funcInfo,
  rootPage,
  onNeedLogin,
  ...
  const estimatePrice = useMemo(() => {
    const price = Number(
      calcPrice(FUNC_NAME.MINIMAX_VIDEO_01, {}).discountPrice ?? 0,
    );
    return price;
  }, []);
  useEffect(() => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    minimaxVideo01(apiKey, data, onFinish, onFail, {
      source: getApiSource(rootPage),
      abortSignal: aborter.current?.signal,
    });
  }, [

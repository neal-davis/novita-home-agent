...
import { minimaxHailuo02, getFailMsgV3, checkProgressV3 } from "@/api/api";
...
const DB_KEY_TASK_ID = "minimax_hailuo_02_task_id";
const DB_KEY_TASK_PROGRESS = "minimax_hailuo_02_task_progress";
const deleteTask = () => {
...
const useCases = defaultCases[FUNC_NAME.MINIMAX_HAILUO_02];
export default function MinimaxHailuo02({
  apiKey,
  funcInfo,
  rootPage,
  onNeedLogin,
  ...
  const [enablePromptExpansion, setEnablePromptExpansion] = useState(
    useCases[0].enable_prompt_expansion,
  );
  ...
  const estimatePrice = useMemo(() => {
    const price = Number(
      calcPrice(FUNC_NAME.MINIMAX_HAILUO_02, {
        duration,
  ...
  useEffect(() => {
    if (duration === 10) {
      setResolution("768P");
      onParamChange?.("resolution", "768P");
    }
  }, [duration, setResolution, onParamChange]);
  useEffect(() => {
    onParamChange?.("image_url", useCases[0].image_url);
    onParamChange?.("prompt", useCases[0].prompt);
    onParamChange?.("duration", 6);
    onParamChange?.("resolution", "768P");
    onParamChange?.(
  ...
  const handleGenerate = useCallback(async () => {
    ...
    minimaxHailuo02(apiKey, data, onFinish, onFail, {
      source: getApiSource(rootPage),
      abortSignal: aborter.current?.signal,
    });
  }, [

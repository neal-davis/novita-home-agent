...
import { wan26I2v, getFailMsgV3, checkProgressV3 } from "@/api/api";
...
const DB_KEY_TASK_ID = "wan26_i2v_task_id";
const DB_KEY_TASK_PROGRESS = "wan26_i2v_task_progress";
const deleteTask = () => {
...
export default function Wan26I2V({
  apiKey,
  funcInfo,
  rootPage,
  onNeedLogin,
  ...
  const estimatePrice = useMemo(() => {
    const price = Number(
      calcPrice(FUNC_NAME.WAN_2_6_I2V, {
  ...
  useEffect(() => {
    setImgUrl(useCases[0].image_url);
    onParamChange?.("prompt", useCases[0].prompt);
    onParamChange?.("img_url", useCases[0].image_url);
    onParamChange?.("resolution", "1080P");
  ...
  const handleGenerate = useCallback(async () => {
    ...
    wan26I2v(apiKey, data, onFinish, onFail, {
      source: getApiSource(rootPage),
      abortSignal: aborter.current?.signal,
    });
  }, [

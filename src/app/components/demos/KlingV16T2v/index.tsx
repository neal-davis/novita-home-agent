...
import { klingV16T2v, getFailMsgV3, checkProgressV3 } from "@/api/api";
...
const DB_KEY_TASK_ID = "kling_v1_6_t2v_task_id";
const DB_KEY_TASK_PROGRESS = "kling_v1_6_t2v_task_progress";
const deleteTask = () => {
...
const useCases = defaultCases[FUNC_NAME.KLING_V1_6_T2V];
export default function KlingV16T2V({
  apiKey,
  funcInfo,
  rootPage,
  onNeedLogin,
  ...
  const estimatePrice = useMemo(() => {
    const price = Number(
      calcPrice(FUNC_NAME.KLING_V1_6_T2V, {
        mode,
        duration,
      }).discountPrice ?? 0,
    );
  ...
  const handleGenerate = useCallback(async () => {
    ...
    klingV16T2v(apiKey, data, onFinish, onFail, {
      source: getApiSource(rootPage),
      abortSignal: aborter.current?.signal,
    });
  }, [

...
import { wan26V2v, getFailMsgV3, checkProgressV3 } from "@/api/api";
...
const DB_KEY_TASK_ID = "wan26_v2v_task_id";
const DB_KEY_TASK_PROGRESS = "wan26_v2v_task_progress";
const deleteTask = () => {
...
const useCases = defaultCases[FUNC_NAME.WAN_2_6_V2V];
export default function Wan26V2v({
  apiKey,
  funcInfo,
  rootPage,
  onNeedLogin,
  ...
  const estimatePrice = useMemo(() => {
    const price = Number(
      calcPrice(FUNC_NAME.WAN_2_6_V2V, {
        resolutionType: size,
        duration,
      }).discountPrice ?? 0,
    );
  ...
  const handleGenerate = useCallback(async () => {
    ...
    const validUrls = referenceVideoUrls.filter((url) => url.trim() !== "");
    if (validUrls.length === 0) {
      message.error("Please enter at least one reference video URL");
      return;
    }
    setGenerating(true);
    taskState.current = TaskState.generating;
    ...
    const data: {
      ...
      input: {
        prompt,
        reference_video_urls: validUrls,
      },
      parameters: {
        size,
        duration,
    ...
    wan26V2v(apiKey, data, onFinish, onFail, {
      source: getApiSource(rootPage),
      abortSignal: aborter.current?.signal,
    });
  }, [

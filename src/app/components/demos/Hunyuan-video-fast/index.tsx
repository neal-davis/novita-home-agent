...
import { hunyuanVideoFast, getFailMsgV3, checkProgressV3 } from "@/api/api";
...
const DB_KEY_TASK_ID = "hunyuan_video_fast_task_id";
const DB_KEY_TASK_PROGRESS = "hunyuan_video_fast_task_progress";
const deleteTask = () => {
...
const useCases = defaultCases[FUNC_NAME.HUNYUAN_VIDEO_FAST];
export default function Txt2Video({
  ...
  const [model, setModel] = useState("hunyuan-video-fast");
  ...
  useEffect(() => {
    onParamChange?.("model_name", "hunyuan-video-fast");
  ...
  const estimatePrice = useMemo(() => {
    ...
    const price = Number(
      calcPrice(FUNC_NAME.HUNYUAN_VIDEO_FAST, {
        model,
        steps,
        resolutionType,
      }).discountPrice ?? 0,
  ...
  const handleGenerate = useCallback(async () => {
    ...
    const data: HunyuanVideoFastRequest = {
      model_name: model,
      width,
      height,
      seed,
    ...
    hunyuanVideoFast(apiKey, data, onFinish, onFail, {
      source: getApiSource(rootPage),
      abortSignal: aborter.current?.signal,
    });
  }, [

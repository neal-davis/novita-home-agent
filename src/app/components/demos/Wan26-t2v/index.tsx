...
import { wan26T2v, getFailMsgV3, checkProgressV3 } from "@/api/api";
...
const DB_KEY_TASK_ID = "wan26_t2v_task_id";
const DB_KEY_TASK_PROGRESS = "wan26_t2v_task_progress";
const deleteTask = () => {
...
const useCases = defaultCases[FUNC_NAME.WAN_2_6_T2V];
export default function Wan26T2v({
  apiKey,
  funcInfo,
  rootPage,
  onNeedLogin,
  ...
  const estimatePrice = useMemo(() => {
    const price = Number(
      calcPrice(FUNC_NAME.WAN_2_6_T2V, {
        duration,
        resolutionType: size,
      }).discountPrice ?? 0,
    );
  ...
  const handleGenerate = useCallback(async () => {
    ...
    wan26T2v(apiKey, data, onFinish, onFail, {
      source: getApiSource(rootPage),
      abortSignal: aborter.current?.signal,
    });
  }, [

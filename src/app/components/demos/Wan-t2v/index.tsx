...
import { ResponseCode, APIErrReason, WanT2vRequest } from "novita-sdk-v3";
import FakeProgress from "fake-progress";
import { UserState } from "@/store/slice/userSlice";
import { fetchBalanceDetail } from "@/store/slice/billingSlice";
import { useAppDispatch, useAppSelector } from "@/store";
import { wanT2v, getFailMsgV3, checkProgressV3 } from "@/api/api";
...
const DB_KEY_TASK_ID = "wan_t2v_task_id";
const DB_KEY_TASK_PROGRESS = "wan_t2v_task_progress";
const deleteTask = () => {
...
const useCases = defaultCases[FUNC_NAME.WAN_T2V];
export default function WanT2V({
  apiKey,
  funcInfo,
  rootPage,
  onNeedLogin,
  ...
  const [model, setModel] = useState("wan2.1-t2v");
  ...
  const estimatePrice = useMemo(() => {
    const resolutionType = `${width}*${height}`;
    const price = Number(
      calcPrice(FUNC_NAME.WAN_T2V, {
        model,
        fps: 16,
        resolutionType,
      }).discountPrice ?? 0,
    );
    return formatMoneyDisplay(price);
  }, [model, width, height]);
  useEffect(() => {
    onParamChange?.("model_name", "wan2.1-t2v");
    onParamChange?.("width", 720);
    onParamChange?.("height", 1280);
      ...
      const onFinish = (
        ...
        if (taskState.current !== TaskState.init) {
          ...
          if (
            ...
            Array.isArray(extra.has_nsfw_contents)
          ) {
            setIsNsfw(extra.has_nsfw_contents[0] || false);
          } else {
            setIsNsfw(false);
          }
  ...
  const handleGenerate = useCallback(async () => {
    ...
    wanT2v(apiKey, data, onFinish, onFail, {
      source: getApiSource(rootPage),
      abortSignal: aborter.current?.signal,
    });
  }, [
    ...
    <DemoWrapper
      ...
      formFoot={
        <FormFooter
          ...
          estimatePrice={estimatePrice as unknown as number}
        />
      }
      resultContent={
        <ResultContent

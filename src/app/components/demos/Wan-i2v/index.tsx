...
import { wanI2v, getFailMsgV3, checkProgressV3 } from "@/api/api";
...
const DB_KEY_TASK_ID = "wan_i2v_task_id";
const DB_KEY_TASK_PROGRESS = "wan_i2v_task_progress";
const deleteTask = () => {
...
const useCases = defaultCases[FUNC_NAME.WAN_I2V];
export default function WanI2V({
  ...
  const [model, setModel] = useState("wan2.1-i2v");
  ...
  useEffect(() => {
    setImageUrl(useCases[0].image_url);
    onParamChange?.("model_name", "wan2.1-i2v");
    onParamChange?.("image_url", useCases[0].image_url);
    onParamChange?.("width", 1280);
    onParamChange?.("height", 720);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    wanI2v(apiKey, data, onFinish, onFail, {
      source: getApiSource(rootPage),
      abortSignal: aborter.current?.signal,
    });
  }, [

...
import styles from "./Dragger_new.module.scss";
  ...
  (props: DraggerProps, ref) => {
    ...
    return (
      ...
      >
        ...
        {uploading && <Loading />}
        {srcImg && (
          <div className={styles.src_delete_btn}>
            <div
              className={styles.src_delete_icon_wrap}
              onClick={(e) => {
                setSrcImg("");
                props.onUpload("", { w: 0, h: 0 });
                e.stopPropagation();
                e.preventDefault();
              }}
            >
              <span
                className={`${styles.delete_icon} iconfont icon-delete`}
              ></span>
            </div>
          </div>
        )}
        ...
        <div
          ...
          className={`${styles.dragger} ${srcImg ? styles.has_img : ""} ${
            uploading ? styles.uploading : styles.not_uploading
          }`}
          onClick={() => {
            if (!props.disabled) {
        ...
        >
          ...
          <div className={styles.dragger_inner}>
            <div
              className={`${styles.dragger_btn} flex justify-center items-center gap-x-2`}
            >
              <span className="iconfont icon-upload"></span>
              Upload Image
            </div>
            {props.desc && (

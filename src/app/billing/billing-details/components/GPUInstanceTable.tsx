...
const GPUInstanceTable = ({
  ...
  useEffect(() => {
    getBillCategory({ productCategory: "gpu" }).then((res) => {
    ...
    if (billList) {
      const tableData: any = billList.map((row: any) => {
        return {
          ...
          ["Pricing Model"]:
            (createCopyPricingModel() as any)[row.billingMethod] || "",
      ...
      XLSX.writeFile(wb, `GPU-Instances-Ondemand-Billing.xlsx`);
    }
  ...
  return (
    <div>
      <div className="mb-[30px] flex flex-row justify-between gap-4 items-end">
        <div className="flex flex-row items-end gap-4 flex-wrap">
          ...
          >
            <SelectTrigger className="w-[180px] text-groupbtn-foreground">

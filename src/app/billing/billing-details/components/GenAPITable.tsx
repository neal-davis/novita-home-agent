  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        header: "Usage",
        render: (row: Bill) => {
          return displayOrDash(row.billNum);
        },
      },
      {
        ...
        header: "Usage Unit",
        render: (row: Bill) => {
          return displayOrDash(row.billNumUnit);
        },
      },
      {
        ...
        render: (row: Bill) => {
          return moneyOrDash(row.originAmountDecimal);
        },
      },
      {
    ...
    if (billList) {
      const tableData: any = billList.map((row: any) => {
        return {
          ...
          ["API Name"]: row.productName,
          ["Request Count"]: row.requestCount || "",
          ["Usage"]: displayOrDash(row.billNum),
          ["Usage Unit"]: displayOrDash(row.billNumUnit),
          ["Original Amount" + EXCEL_MONEY_HEADER_SUFFIX]: exportMoneyOrDash(
            row.originAmountDecimal,
          ),
      ...
      const ws = XLSX.utils.json_to_sheet(tableData, {
        header: [
          ...
          "Usage",
          "Usage Unit",
          "Original Amount" + EXCEL_MONEY_HEADER_SUFFIX,
      ...
      XLSX.writeFile(wb, `Image-Video-Ondemand-Billing.xlsx`);
    }

  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (row: any) => {
          return `${getDateDisplay(Number(row.startTime), "hour")}`;
        },
      },
      {
        accessorKey: "endTime",
        ...
        className: "min-w-[180px]",
        render: (row: any) => {
          return `${getDateDisplay(Number(row.endTime), "hour")}`;
        },
      },
      {
    ...
    if (billList) {
      const tableData: any = billList.map((row: any) => {
        return {
          ...
          ["Instance ID"]: `${row.ownerID || ""}`,
          ["Pricing Model"]: `${tradeModes[row.tradeMode] || ""}`,
          ["Billing Type"]: `${tradeTypes[row.tradeType] || ""}`,
          ["Start Time"]: `${getDateDisplay(Number(row.startTime), "hour")}`,
          ["End Time"]: `${getDateDisplay(Number(row.endTime), "hour")}`,
          ["Monthly Rate($)"]: formatBillingPrice(
            row.basePrice,
            row.pricePrecision,
          ),
          ["Usage"]: `${row.billNum}`,
      ...
      XLSX.writeFile(wb, `GPU-Instance-Monthly-Billing.xlsx`);
    }
  }, [billList, getCreator]);
...
export default GPUInstanceTableMonthly;

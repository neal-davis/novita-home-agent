...
const formatSandboxUnitPrice = (
  price: number | string,
  pricePrecision?: number | string,
) => formatBillingPrice(price, pricePrecision);
...
const SandboxTable = ({
  ...
  const columns = useMemo(
    () => [
      ...
      //   render: (row: any) => getDateDisplay(Number(row.createTime), "hour"),
      // },
      {
      ...
      },
      {
        ...
        render: (row: any) => {
          return (
            Math.round((Number(row.billNum0 || 0) / 3600 / 1024) * 10000) /
            10000
          ).toFixed(4);
        },
      },
      {
        accessorKey: "RAM Hours",
        header: "RAM Hours",
        render: (row: Bill) => {
          return (
            Math.round((Number(row.billNum1 || 0) / 3600 / 1024) * 10000) /
            10000
          ).toFixed(4);
        },
      },
      {
        accessorKey: "vCPU Price",
        header: "vCPU Price(/core/sec)",
        render: (row: any) => {
          return (
            "$" + formatSandboxUnitPrice(row.discountPrice0, row.pricePrecision)
          );
        },
      },
      {
        accessorKey: "RAM Price",
        header: "RAM Price(/GiB/sec)",
        render: (row: Bill) => {
          return (
            "$" + formatSandboxUnitPrice(row.discountPrice1, row.pricePrecision)
          );
        },
      },
      {
        accessorKey: "Billing Mode",
        header: "Billing Mode",
        render: () => {
  ...
  const exportFile = useCallback(() => {
    ...
    if (billList) {
      const tableData: any = billList.map((row: any) => {
        return {
          "Service Period": getDateRangeDisplay(
            filterOptions.cycleType,
            row?.startTime,
            row?.endTime,
          ),
          ...
          ["vCPU Price(/core/sec)" + EXCEL_MONEY_HEADER_SUFFIX]:
            formatSandboxUnitPrice(row.discountPrice0, row.pricePrecision),
          ["RAM Price(/GiB/sec)" + EXCEL_MONEY_HEADER_SUFFIX]:
            formatSandboxUnitPrice(row.discountPrice1, row.pricePrecision),
      ...
      XLSX.writeFile(wb, `Agent-Sandbox-Ondemand-Billing.xlsx`);
    }
          ...
          <TableRow>
            {columns.map((column, index) => (
              <TableHead
                ...
                className={
                  index === 0 ? styles.fixed_first_column_header : undefined
                }
              >
                {column.header}
              </TableHead>

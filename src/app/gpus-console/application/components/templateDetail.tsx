...
export default function TemplateDetail({
  ...
  templateId: string;
  deployApplication: (item: any) => void;
}) {
  ...
  return !loading ? (
    <div>
      <div className="flex flex-row items-center gap-2 mb-6">
        <div className="inline-block font-body-medium text-[var(--black)]">
          {"Application"}
        </div>
        ...
        <div
          ...
          hover:cursor-pointer hover:bg-[var(--gray-3)] font-menu text-[var(--black)]"
        >
          {templateInfo.type && templateInfo.type.length
            ? templateInfo.type.charAt(0).toUpperCase() +
              templateInfo.type.slice(1)
            : "All"}
        </div>
        <ChevronRight className="w-[14px] h-[14px] text-[var(--dark-2)]" />
      ...
      <div className={styles.template_detail_container}>
          ...
          <div className="flex flex-col gap-2">
            <div className="flex flex-row gap-[26px] items-center mb-2">
              ...
              <div className="flex items-center justify-center gap-3">
                {["official"].includes(templateInfo.channel) && (
                  <Tooltip
                    title={`${templateInfo.isCollected ? "Unfavorite Application" : "Favorite Application"}`}
                  >
                    <div
            ...
            <div className="flex flex-row items-center gap-4">
              <div className="flex flex-row items-center gap-2">
                ...
                {templateInfo.enableApplicationInstance &&
                  templateInfo.type && (
                    <div
                      className="h-5 flex items-center justify-center px-[6px] bg-[var(--gray-3)] 
                  rounded-[4px] font-small-console text-[var(--dark-1)]"
                    >
                ...
                {templateInfo.enableApplicationServerless && (
                  <div
                    className="h-5 flex items-center justify-center px-[6px] bg-[var(--gray-3)] 
                  rounded-[4px] font-small-console text-[var(--dark-1)]"
                  >
        ...
        <Button
          variant="default"
          size="lg"
          onClick={() => deployApplication(templateInfo)}
        >
          <span className="font-subtle-medium mr-1">{"Deploy"}</span>
          <ChevronRight className="w-4 h-4 text-[var(--white)]" />
        </Button>
      </div>

...
import SandboxTable from "@/app/billing/billing-details/components/Sandbox";
...
// billNum0 vCPU-seconds -> hours: 7372800 / 3600 / 1024 = 2.0000
const bill = {
  billNum0: 7372800,
  billNum1: 3686400,
  discountPrice0: 5000,
  discountPrice1: 10000,
...
describe("SandboxTable", () => {
  ...
  it("renders vCPU/RAM hours, prices, billing mode, and money columns", async () => {
    render(<SandboxTable pageSize={10} />);

    expect(await screen.findByText("2.0000")).toBeInTheDocument();
    expect(screen.getByText("1.0000")).toBeInTheDocument();
    expect(screen.getByText("Pay-as-you-go")).toBeInTheDocument();
    expect(screen.getByText("vCPU Hours")).toBeInTheDocument();
    expect(screen.getByText("RAM Hours")).toBeInTheDocument();
    ...
    expect(getBillListMock).toHaveBeenCalledWith(
      expect.objectContaining({ productCategory: "cloud_sandbox" }),
      expect.any(AbortSignal),
    );
  });

  it("exports the sandbox workbook", async () => {
    render(<SandboxTable pageSize={10} />);
    await screen.findByText("2.0000");

    fireEvent.click(screen.getByRole("button", { name: "Export" }));
    expect(XLSX.utils.json_to_sheet).toHaveBeenCalledWith(
      expect.arrayContaining([
        expect.objectContaining({
          ["vCPU Price(/core/sec)($)"]: "0.00005",
          ["RAM Price(/GiB/sec)($)"]: "0.0001",
        }),
      ]),
      expect.any(Object),
    );
    expect(XLSX.writeFile).toHaveBeenCalledWith(
      expect.any(Object),
      "Agent-Sandbox-Ondemand-Billing.xlsx",
    );
  });
  ...
  it("refetches on date change and pseudo-paginates locally", async () => {
    render(<SandboxTable pageSize={10} />);
    await screen.findByText("2.0000");

    fireEvent.click(screen.getByRole("button", { name: "pick date" }));
    await waitFor(() => expect(getBillListMock).toHaveBeenCalledTimes(2));
    ...
    expect(getBillListMock).toHaveBeenCalledTimes(2);
    jest.useRealTimers();
  });

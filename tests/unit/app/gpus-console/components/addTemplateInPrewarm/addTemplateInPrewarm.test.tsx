...
jest.mock("@/app/gpus-console/settings/components/imageAuth", () => ({
  __esModule: true,
  default: () => <div>image-auth-form</div>,
}));
...
jest.mock("@/lib/utils/utils", () => ({
  ...
  generateRandomString: jest.fn(() => "prewarm-template"),
}));
...
describe("AddTemplateInPrewarm", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockReqAddTemplate.mockResolvedValue({ templateId: "template-from-image" });
    ...
    mockReqGetImageAuths.mockResolvedValue({
      data: [{ id: "auth-7", name: "Private registry" }],
    });
  });

  it("creates a template from a prewarmed image with the image auth preselected", async () => {
    const finishForm = jest.fn();
    const { container } = render(
      <AddTemplateInPrewarm
        authId="auth-7"
        finishForm={finishForm}
        imageUrl="private.registry/team/app:1.0"
        mode="Create"
      />,
    );
    ...
    expect(
      screen.getByDisplayValue("private.registry/team/app:1.0"),
    ).toBeInTheDocument();
    expect(screen.getAllByTestId("select-filter")[1]).toHaveAttribute(
      "data-value",
      "auth-7",
    );
    ...
    await waitFor(() => {
      expect(mockReqAddTemplate).toHaveBeenCalledWith({
        template: expect.objectContaining({
          channel: "private",
          image: "private.registry/team/app:1.0",
          imageAuth: "auth-7",
          name: "prewarm-template",
          rootfsSize: "40",
          volumes: [],
        }),
      });
